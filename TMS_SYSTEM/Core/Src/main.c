...
void Smooth_LED_Task(void *pvParameters);
void Static_LED_Task(void *pvParameters);
...
osThreadId_t S_LEDTaskHandle;
const osThreadAttr_t S_LEDTask_attributes = {
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOC, STOP_SIGN2_Pin|STOP_SIGN_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pin : STOP_SIGN2_Pin */
  GPIO_InitStruct.Pin = STOP_SIGN2_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
  HAL_GPIO_Init(STOP_SIGN2_GPIO_Port, &GPIO_InitStruct);
  ...
  /*Configure GPIO pin : STOP_SIGN_Pin */
  GPIO_InitStruct.Pin = STOP_SIGN_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(STOP_SIGN_GPIO_Port, &GPIO_InitStruct);
...
void Servo_Task(void *pvParameters)
{
  while(1)
  {
	    htim3.Instance->CCR1 = Servo_0;
	    HAL_Delay(1900 - (SPEED_STATUS * 40));
	  	htim3.Instance->CCR1 = Servo_90;
	  	HAL_Delay(1900 - (SPEED_STATUS * 40));
	  	htim3.Instance->CCR1 = Servo_180;
	  	HAL_Delay(1900 - (SPEED_STATUS * 40));
	  	htim3.Instance->CCR1 = Servo_90;
	  	HAL_Delay(1900 - (SPEED_STATUS * 40));
  }
...
void Smooth_LED_Task(void *pvParameters)
{
    while (1)
    {
    	while(1){
    		...
    		htim3.Instance->CCR3 = LED_Value;
    		LED_Value=LED_Value+6;
...
void Static_LED_Task(void *pvParameters)
{
	while(1){
		...
		if(STOP_STATUS == 1){
			HAL_GPIO_WritePin(STOP_SIGN_GPIO_Port,STOP_SIGN_Pin , GPIO_PIN_SET);
			HAL_GPIO_WritePin(STOP_SIGN2_GPIO_Port,STOP_SIGN2_Pin , GPIO_PIN_RESET);
		}
		else{
	        HAL_GPIO_WritePin(STOP_SIGN_GPIO_Port,STOP_SIGN_Pin , GPIO_PIN_RESET);
	        HAL_GPIO_WritePin(STOP_SIGN2_GPIO_Port,STOP_SIGN2_Pin , GPIO_PIN_SET);
		}
	}
}
...
void StartDefaultTask(void *argument)
{
  ...
  LEDTaskHandle = osThreadNew(Smooth_LED_Task,NULL,&LEDTask_attributes);
  S_LEDTaskHandle = osThreadNew(Static_LED_Task,NULL,&S_LEDTask_attributes);
  /* Infinite loop */
  for(;;)
  {
	  if (STOP_STATUS){
		  ...
		  vTaskResume(ServoTaskHandle); //LIG NEX1 1�? ?��?���?
	  }
  }
  /* USER CODE END 5 */
}
